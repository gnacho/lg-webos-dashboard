...
The **Advanced** tab, `/?tab=advanced`, holds the TV's own settings that LG keeps several menus deep: Quick Boot, Wake-on-LAN, the LG logo shown at power on and off, auto device detection, and the front lights. Auto device detection looks for a set-top box the Magic Remote can control, and on newer TVs does it by looking up every address on the home network each time the TV switches on; switching it off stops those lookups. Each setting appears only on TVs that have it. On the TV itself they are under **System**.
